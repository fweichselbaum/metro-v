...
import { findStopByID, getMarkerFromStop, getPolylineFromLine } from './src/service/MetroService';
      ...
      {menuOpen &&
        metroLines.map((line, i) => (
          <MenuItem
            ...
            onSwitchChanged={() =>
              setSelectedLines(
                selectedLines.map((selectedLine, j) => (i == j ? !selectedLine : selectedLine))
              )
            }
          />
        ))}

      <TouchableNativeFeedback style={styles.button} onPress={() => setMenuOpen(!menuOpen)}>
        <View style={styles.buttonContainer}>
          <Text style={styles.buttonLabel}>Filter Metros</Text>
          <Ionicons style={styles.buttonIcon} name={menuOpen ? 'chevron-down' : 'chevron-up'} />
        </View>
      </TouchableNativeFeedback>
    </View>
  );
